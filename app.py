...
DB_CONFIG = {
    ...
    'database': 'StudySync',
    'user': 'postgres',
    'password': 'Aylward2015*'
    }
# Email Configuration
EMAIL_HOST = 'smtp.gmail.com'
EMAIL_PORT = 587
EMAIL_USERNAME = 'elhamriamine2003@gmail.com'
EMAIL_PASSWORD = 'pgzi gijd zqbn idxl'      # Replace with 16-char app password
EMAIL_FROM = 'StudySync <elhamriamine2003@gmail.com>'
...
def save_availability():
    """Save user's availability for a group"""
...
def get_availability(group_id):
    """Get all availability for a group"""
    conn = get_db_connection()
    if not conn:
        return jsonify({'success': False, 'error': 'Database error'}), 500
...
def find_optimal_times(group_id):
    """Run CSP algorithm to find optimal meeting times"""
    ...
    try:
        ...
        if len(availability_data) < 2:
            return jsonify({
                'success': False,
                'error': 'Need at least 2 members to submit availability'
            }), 400
...
def run_csp_with_travel(availability_data, travel_times):
    """
    Constraint Satisfaction Algorithm for finding optimal meeting times
    """
...
def import_calendar_availability(group_id):
    """Import availability from Google Calendar for a specific date range"""
    data = request.json
    user_id = data.get('user_id')
    start_date_str = data.get('start_date')  # ISO format
    end_date_str = data.get('end_date')      # ISO format
    ...
    try:
        ...
        # Save to database
        # Delete existing availability for this user/group
        cur.execute(
            "DELETE FROM availability WHERE group_id = %s AND user_id = %s",
            (group_id, user_id)
        )
...
def get_group_location(group_id):
    """Get meeting location for a group"""
    conn = get_db_connection()
    if not conn:
        return jsonify({'success': False, 'error': 'Database error'}), 500
...
def create_meeting():
    """Schedule a meeting from optimal time"""
    data = request.json
    group_id = data.get('group_id')
    day_of_week = data.get('day_of_week')
    meeting_time = data.get('meeting_time')
    meeting_date = data.get('meeting_date')  # Optional
    ...
    try:
        # Get group details including location
        cur.execute("""
    ...
    try:
        ...
        for row in cur.fetchall():
            meetings.append({
                'id': row[0],
                'day_of_week': row[1],
                'meeting_time': str(row[2]),  # Convert time to string
...
def get_user_meetings(user_id):
    """Get all meetings for a user across all their groups"""
    conn = get_db_connection()
    if not conn:
        return jsonify({'success': False, 'error': 'Database error'}), 500
